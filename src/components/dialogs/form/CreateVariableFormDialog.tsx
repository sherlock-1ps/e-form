...
import { Divider, Grid, IconButton, InputAdornment, MenuItem, Switch, Typography } from '@mui/material'
...
const CreateVariableFormDialog = ({ id, onClick }: createFormProps) => {
  ...
  const { mutateAsync } = useCreateVariableQueryOption()
  ...
  const handleCreateVariable = async () => {
    ...
    try {
      ...
      if (response?.code == 'SUCCESS') {
        toast.success('เพิ่ม Variable สำเร็จ!', { autoClose: 3000 })
        closeDialog(id)
      }
    } catch (error) {
      console.error('❌ create failed', error)
      toast.error('เพิ่ม Variable ล้มเหลว!', { autoClose: 3000 })
    }
  }
...
export default CreateVariableFormDialog
