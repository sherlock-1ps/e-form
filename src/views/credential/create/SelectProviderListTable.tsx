...
const SelectProviderListTable = ({ dataTable, category, updateMain }: any) => {
  ...
  const handleChangeShareCredential = (share: string, index: number) => {
    ...
    const cleanedShare = Math.max(Number(share), 0) // Convert to number and force minimum 0
    const percentHolder = data[index]?.credential_percent || 0

    const validShare = Math.min(cleanedShare, percentHolder) // Cap at credential_percent

    updatedData[index] = {
      ...updatedData[index],
      selectShare: validShare
    }

    updateMain(category, index, validShare, undefined)
  ...
  const columns = useMemo<ColumnDef<ProviderCredentialType, any>[]>(
    () => [
      ...
      columnHelper.display({
        id: 'provider',
        header: dictionary['credential']?.selectAllProvider,
        enableSorting: false,
        cell: ({ row }) => (
                        ...
                        <td key={cell.id}>
                          ...
                          {index == 3 && (
                            <Typography variant='h6'>
                              {row.original.credential_percent - Number(dataModify[row.index]?.selectShare)}%
                            </Typography>
                          )}
                          {index == 4 && dataModify[row.index]?.is_select && (
                            <CustomTextField
                              ...
                              onChange={e => handleChangeShareCredential(e.target.value, row.index)}
                              onKeyDown={e => {
